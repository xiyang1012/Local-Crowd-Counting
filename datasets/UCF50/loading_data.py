...
def SHHA_collate_multibatch(batch):
...
def SHHA_collate_onebatch(batch):
    transposed = list(zip(*batch))  # imgs and dens
    imgs, dens = [transposed[0], transposed[1]]
...
def loading_data():
    ...
    train_set = UCF50(cfg_data.DATA_PATH, train_folder, 'train',
    ...
    val_set = UCF50(cfg_data.DATA_PATH, map(int,str(val_folder)), 'test',
                    main_transform=None,
                    img_transform=img_transform,
                    gt_transform=gt_transform)
